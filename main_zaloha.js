...
canvas.addEventListener('mousedown', function(event){
    ...
    if(gameMng.clicked == 1){
        ...
        if(gameMng.round == 1){
            ...
            let font = getPercent(lowerCanvas.width, 2)+'px Verdana';
            lowerCtx.font = font;
            lowerCtx.fillText('Vybrali ste si hlavné mesto: '+ hglTiles[0].playerColor, getPercent(lowerCanvas.width,10), lowerCanvas.height - 8);
            gameMng.round++;
        }
        else{
            getClicked(mouse.x, mouse.y, tiles);
        ...
        //const tile = new Tile(mouse.x, mouse.y);
        //tile.draw();
        //tiles.push(tile);
        lowerCtx.fillStyle = 'white';
...
//Cursor for moving and direction orientation
class TCursor {
    constructor(troop){
        this.cursorx = troop.x,
        this.cursory = troop.y,
        
        this.radiusRate = function(rate){
            rate = (_radius/100)*rate;
            return rate;
        }
    
        this.heightRate = function(rate){
            rate = (_height/100)*rate;
            return rate;
        }

        this.yLimit = this.heightRate(40);
        this.xLimit = this.radiusRate(70)
        this.moveToX = this.cursorx+this.xLimit;
        this.moveToY = this.cursory;
        this.firstPX = this.cursorx-this.xLimit;
        this.firstPY = this.cursory - this.yLimit;
        this.secondPX = this.cursorx-this.xLimit;
        this.secondPY = this.cursory + this.yLimit;
        this.draw = function(){
            ctx.fillStyle = 'white';
            ctx.beginPath();
            ctx.moveTo(this.moveToX, this.moveToY);
            ctx.lineTo(this.firstPX, this.firstPY);
            ctx.lineTo(this.secondPX, this.secondPY);
            /*ctx.moveTo(this.cursorx+this.radiusRate(70), this.cursory);
            ctx.lineTo(this.cursorx-this.radiusRate(50), this.cursory - this.heightRate(40));
            //console.log('point x: ' + this.cursorx+this.radiusRate(70));
            ctx.lineTo(this.cursorx-this.radiusRate(50), this.cursory + this.heightRate(40));*/
            ctx.fill();
            ctx.closePath();
        ...
        this.turnUp = function(){
            if(this.moveToX > this.cursorx){
                
                this.moveToX = this.moveToX - this.radiusRate(10);
                this.moveToY = this.moveToY - (this.yLimit/2);
                this.firstPX = this.firstPX - this.radiusRate(10);
                this.firstPY = this.cursory;
                this.secondPX = this.secondPX + 200;
                this.secondPY = this.secondPY + 200;
...
class Circle{
    ...
    draw(radius){        
        ...
        if(this.type == 'capital' || this.type == 'violet' || this.type == 'green' || this.type == 'blue'){
            ...
            ctx.moveTo(this.x + (radius/100)*65, this.y);
            ctx.lineWidth = 3;
            ctx.arc(this.x, this.y, (radius/100)*65, 0, Math.PI * 2, true);
